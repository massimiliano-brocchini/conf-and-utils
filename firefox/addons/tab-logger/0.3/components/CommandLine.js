const CATEGORY = 'c-tablogger';
const CONTRACT_ID = '@mozilla.org/commandlinehandler/general-startup;1?type=tl';


const Cc = Components.classes;
const Ci = Components.interfaces;
const Cr = Components.results;
const CLASS_ID = Components.ID('{e7be3d20-36c0-11e0-9e42-0800200c9a66}');
...
const Handler = {
    ...
    handle: function(cmdLine) {
        var uri;
        try {
            uri = cmdLine.handleFlagWithParam('tl', false);
        } catch (e) {
        }

        if(uri || cmdLine.handleFlag('tl', false))
            Cc['@massimiliano.brocchini/max-repo/tablogger;1']
            .getService(Ci.nsITabLogger)
            .start(uri ? parseInt(uri) : 4242);
    },

    helpInfo: '-tl              Start tl.\n',
