...
function initOverlay() {
    server = Cc['@hyperstruct.net/mozlab/tablogger;1'].getService(Ci.nsITabLogger);

    // upgradeCheck(
    //     'tablogger@hyperstruct.net',
    //     'extensions.tablogger.version', {
...
function updateMenu(xulPopup) {
    document.getElementById('tablogger-command-toggle')
        .setAttribute('label', server.isActive() ? 'Stop' : 'Start');
    document.getElementById('tablogger-command-listen-external')
        .setAttribute('checked', !pref.getBoolPref('loopbackOnly'));
    document.getElementById('tablogger-command-autostart')
...
function openHelp() {
    openURL('http://github.com/bard/tablogger/wikis/home');
}
